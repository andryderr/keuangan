              ...
              <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>{{$row->tgl_masuk}}</td>
                    <td>
                      <button href="#" data-toggle="popover" data-html="true" id="pop{{$row->id_detail_pendaftaran}}" title="Pilihan" data-content="" onclick="over({{$row->id_pendaftaran.','.$row->id_detail_pendaftaran.','.$row->pendaftaran->no_rm}});" data-placement="left" class="btn btn-primary"><span class="fa fa-check"></span></button>
                    </td>
                  </tr>
                  @endforeach                  
                </div>
...
<a href="#" data-toggle="tooltip" title="Informasi Pasien">
<button style="width: 37px;" id="a1" type="button" data-toggle="modal" data-target="#editPendaftaran" class="btn btn-info"><i class="fa fa-circle"></i></button></a>
    <a id="a2" data-toggle="tooltip" title="Tindakan & Diagnosa Pasien"><button type="button" style="width: 35px;" class="btn btn-success"><i class="fa fa-stethoscope"></i></button></a>
    ...
    <!-- <a href="#" data-toggle="tooltip" title="Keluar"><button type="button" id="a6" style="width: 37px;" data-toggle="modal" data-target="#pasien_keluar" class="btn btn-warning"><i class="fa fa-sign-out"></i></button></a> -->
  </div>
</div>
...
<script>
  ...
  function over(id,id2,id3){
    ...
    $("#a2").attr('href',"{{url(Auth::user()->poli->prefix.'/dataPasien/diagnosaPasien1/')}}/"+id+"/"+id2);
    ...
    // $("#a6").attr('href',"{{url(Auth::user()->poli->prefix.'/dataPasien/Pemeriksaan')}}/"+id2);
    $("#a6").attr('onclick',"$('#id_detail_pendaftaran').val("+id2+")");
    $("#a7").attr('href',"{{url(Auth::user()->poli->prefix.'/dataPasien/Pemeriksaan/')}}/"+id2)
    // $("#a7").attr("href","{{url(Auth::user()->poli->prefix.'/dataPasien/riwayat')}}/"+id3);
    $("#pop"+id2).attr('data-content',$("#test").html());
  }
</script>
@endsection
