   ...
   <script src="{{url('/plugins/moment-develop/min/moment.min.js')}}"></script>
    ...
    <script type="text/javascript">
        function editPasien(id)
        {
                    ...
                    $.ajax({
                        url : "<?php echo site_url('pendaf/pendaftaran/editPasien/')?>/" +id,
                        type: "GET",
                        dataType: "JSON",
                        success: function(data)
                        {
                            $('[name="noRM"]').val(data.);
                            $('[name="namaPasien"]').val(data.);
                            $('[name="jamLahir"]').val(data.);
                            $('[name="tanggalLahir"]').datepicker('update',data.);
                            $('[name="alamat"]').val(data.);
                            $('[name="kecamatan"]').val(data.);
                            $('[name="desa"]').val(data.);
                            $('[name="dusun"]').val(data.);
                            $('[name="tempatLahir"]').val(data.);
                            $('[name="beratLahir"]').val(data.);
                            $('[name="golonganDarah"]').val(data.);
                            $('[name="agama"]').val(data.);
                            $('[name="status"]').val(data.);
                            $('[name="pekerjaan"]').val(data.);
                            $('[name="jenisIdentitas"]').val(data.);
                            $('[name="noIdentitas"]').val(data.);
                            $('[name="telepon"]').val(data.);
                            $('[name="namaOrangTua"]').val(data.);
                            $('[name="namaSuamiIstri"]').val(data.);
                            $('#editPasien').modal('show'); // show bootstrap modal when complete loaded
                        },
